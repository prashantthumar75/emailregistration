...
# from teams.api.models import TeamInvitation
# from proj.teams.api.serializers import TeamSerializer
# from proj.teams.models import TeamInvitation
...
class UserLoginSerializer(serializers.ModelSerializer):
    ...
    class Meta(object):
        model = User
        fields = ['email', 'username', 'password', 'token','first_name']
    ...
    def validate(self, data):
        ...
        if not email and not username and not first_name:
            raise serializers.ValidationError("Please enter username or email and first_name to login.")

        user = User.objects.filter(
            Q(email=email) | Q(username=username) | Q(first_name=first_name)
        ).exclude(
            email__isnull=True
        ).exclude(
            email__iexact=''
        ...
        if user.exists() and user.count() == 1:
            ...
            raise serializers.ValidationError("This username/email/name is not valid.")
